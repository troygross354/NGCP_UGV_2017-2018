...
function readInfo()
    local data=simReadCustomDataBlock(model,'XYZ_PILLOWBAG_INFO')
    if data then
...
function writeInfo(data)
    if data then
        simWriteCustomDataBlock(model,'XYZ_PILLOWBAG_INFO',simPackTable(data))
    else
        simWriteCustomDataBlock(model,'XYZ_PILLOWBAG_INFO','')
    end
end
...
function setColor(red,green,blue,spec)
    simSetShapeColor(convex,nil,sim_colorcomponent_ambient_diffuse,{red,green,blue})
end

function getColor()
    local r,rgb=simGetShapeColor(convex,nil,sim_colorcomponent_ambient_diffuse)
...
function updateModel()
    ...
    if simBoolAnd32(bitCText,4)>0 then
        -- textured
        simSetShapeTexture(convex,textureId,sim_texturemap_plane,4+8,{0.3,0.3})
    else
        -- without texture
        simSetShapeTexture(convex,-1,sim_texturemap_plane,4+8,{0.3,0.3})
    end
    
    -- Now the label:

    -- Remove the current labels:
    local objs=simGetObjectsInTree(model,sim_object_shape_type,1+2)
    for i=1,#objs,1 do
        local h=objs[i]
    ...
    if simBoolAnd32(bitC,8)>0 then
        local useLargeLabel=(simBoolAnd32(bitC,64)>0)
        ...
        simSetObjectSpecialProperty(h,sim_objectspecialproperty_detectable_all+sim_objectspecialproperty_renderable) -- make renderable and detectable
...
function onCloseClicked()
    if sim_msgbox_return_yes==simMsgBox(sim_msgbox_type_question,sim_msgbox_buttons_yesno,'Finalizing the pillow bag',"By closing this customization dialog you won't be able to customize the pillow bag anymore. Do you want to proceed?") then
        finalizeModel=true
    ...
    if (not ui) and bwUtils.canOpenPropertyDialog() then
        ...
        <tabs id="77">
            ...
            <tab title="Colors/Texture" layout="form">
                    <checkbox text="Textured" onchange="texture_callback" id="888" />
                    <label text=""/>
...
if (sim_call_type==sim_customizationscriptcall_initialization) then
    ...
    writePartInfo(data)
    texture=simGetObjectHandle('genericPillowBag_texture')
    smallLabel=simGetObjectHandle('genericPillowBag_smallLabel')
    largeLabel=simGetObjectHandle('genericPillowBag_largeLabel')
    convex=simGetObjectHandle('genericPillowBag_convex')
