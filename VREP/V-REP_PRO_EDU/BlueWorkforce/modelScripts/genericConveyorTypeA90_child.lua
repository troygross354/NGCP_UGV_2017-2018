...
if (sim_call_type==sim_childscriptcall_actuation) then
    ...
    simSetObjectFloatParameter(endPad1,sim_shapefloatparam_texture_y,totShift)
    simSetObjectFloatParameter(endPad2,sim_shapefloatparam_texture_y,totShift)

    relativeLinearVelocity={0,beltVelocity,0}
    
    simResetDynamicObject(endPad1)
    m=simGetObjectMatrix(endPad1,-1)
    m[4]=0
    m[8]=0
    m[12]=0
    absoluteLinearVelocity=simMultiplyVector(m,relativeLinearVelocity)
    simSetObjectFloatParameter(endPad1,sim_shapefloatparam_init_velocity_x,absoluteLinearVelocity[1])
    simSetObjectFloatParameter(endPad1,sim_shapefloatparam_init_velocity_y,absoluteLinearVelocity[2])
    simSetObjectFloatParameter(endPad1,sim_shapefloatparam_init_velocity_z,absoluteLinearVelocity[3])

    simResetDynamicObject(endPad2)
    m=simGetObjectMatrix(endPad2,-1)
    m[4]=0
    m[8]=0
    m[12]=0
    absoluteLinearVelocity=simMultiplyVector(m,relativeLinearVelocity)
    simSetObjectFloatParameter(endPad2,sim_shapefloatparam_init_velocity_x,absoluteLinearVelocity[1])
    simSetObjectFloatParameter(endPad2,sim_shapefloatparam_init_velocity_y,absoluteLinearVelocity[2])
    simSetObjectFloatParameter(endPad2,sim_shapefloatparam_init_velocity_z,absoluteLinearVelocity[3])
