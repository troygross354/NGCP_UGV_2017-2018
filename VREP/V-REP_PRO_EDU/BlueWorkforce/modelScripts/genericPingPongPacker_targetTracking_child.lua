...
if (sim_call_type==sim_childscriptcall_initialization) then
    ...
    trackingWindowModel=simGetObjectAssociatedWithScript(sim_handle_self)
    local modelData=simReadCustomDataBlock(model,'CONVEYOR_CONF')
    modelData=simUnpackTable(modelData)
    locationName=modelData['locationName']

    showPoints=bwUtils.modifyAuxVisualizationItems(simBoolAnd32(modelData['bitCoded'],128)>0)
...
if (sim_call_type==sim_childscriptcall_sensing) then
    ...
    local data=simReadCustomDataBlock(trackingWindowModel,'XYZ_TRACKINGWINDOW_INFO')
    ...
    if 1==smallestStage then -- full
        local dat=simReadCustomDataBlock(model,'CONVEYOR_CONF')
        dat=simUnpackTable(dat)
        dat['putCartridgeDown'][currentCartridgeIndex]=true
        simWriteCustomDataBlock(model,'CONVEYOR_CONF',simPackTable(dat))
    ...
    simWriteCustomDataBlock(trackingWindowModel,'XYZ_TRACKINGWINDOW_INFO',simPackTable(data))
