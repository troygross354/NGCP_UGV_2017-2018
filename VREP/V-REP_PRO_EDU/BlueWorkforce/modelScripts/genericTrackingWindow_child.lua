...
simpleDisplayParts=function(parts)
    simAddDrawingObjectItem(sphere3Container,nil)
    for key,value in pairs(parts) do
        local p=value['pickPos']
        simAddDrawingObjectItem(sphere3Container,{p[1],p[2],p[3],0,0,1})
    end
end

displayParts=function(trackingWindowParts)
    simAddDrawingObjectItem(sphere1Container,nil)
--    simAddDrawingObjectItem(sphere2Container,nil)
    for i=1,4,1 do
        simAddDrawingObjectItem(decorationContainers[i],nil)
    end
    simAddDrawingObjectItem(lineContainerR,nil)
    simAddDrawingObjectItem(lineContainerG,nil)
    simAddDrawingObjectItem(lineContainerB,nil)
    local al=0.06
    for key,value in pairs(trackingWindowParts) do
        ...
        if decoration then
            for i=1,#decoration,1 do
                ...
                local stage=simBoolAnd32(decoration[i]['processingStage'],1+2)
                local p=simGetObjectPosition(h,-1)
                simAddDrawingObjectItem(decorationContainers[stage+1],{p[1],p[2],p[3],0,0,1})
            end
        else
            local p=value['pickPos']
            local a=value['axes']
            simAddDrawingObjectItem(sphere1Container,{p[1],p[2],p[3],0,0,1})
            simAddDrawingObjectItem(lineContainerR,{p[1],p[2],p[3]+0.001,p[1]+a[1][1]*al,p[2]+a[1][2]*al,p[3]+a[1][3]*al+0.001})
            simAddDrawingObjectItem(lineContainerG,{p[1],p[2],p[3]+0.001,p[1]+a[2][1]*al,p[2]+a[2][2]*al,p[3]+a[2][3]*al+0.001})
            simAddDrawingObjectItem(lineContainerB,{p[1],p[2],p[3]+0.001,p[1]+a[3][1]*al,p[2]+a[3][2]*al,p[3]+a[3][3]*al+0.001})
        end
    end
end
...
displayTargets=function(targets)
    ...
    for key,value in pairs(targets) do
        local stage=simBoolAnd32(value['processingStage'],1+2)
        local p=simGetObjectPosition(key,-1)
        simAddDrawingObjectItem(decorationContainers[stage+1],{p[1],p[2],p[3],0,0,1})
    end
end

displayGreyTargets=function(targets)
    simAddDrawingObjectItem(decorationContainerGrey,nil)
    for key,value in pairs(targets) do
        local p=simGetObjectPosition(key,-1)
        simAddDrawingObjectItem(decorationContainerGrey,{p[1],p[2],p[3],0,0,1})
    end
end
...
getAllPartsFromInput=function()
    ...
    if inputHandle~=-1 then
        local data=simReadCustomDataBlock(inputHandle,'XYZ_DETECTIONWINDOW_INFO')
        if data then
            data=simUnpackTable(data)
            dat=data['detectedItems']
        else
            data=simReadCustomDataBlock(inputHandle,'XYZ_TRACKINGWINDOW_INFO')
            if data then
                data=simUnpackTable(data)
...
attachDummiesAndDecorate=function(part,partData)
    local h=simCreateDummy(0.005)
    simSetObjectParent(h,model,true)
    simSetObjectInt32Parameter(h,sim_objintparam_visibility_layer,1024)
    
--    orientDummyAccordingToAxes(h,partData['axes'])
    simSetObjectQuaternion(h,-1,partData['transform'][2])

    partData['dummyHandle']=h
    if not partData['decorationInfo'] then
        local data=nil
        if overridePallet then
            data=simReadCustomDataBlock(model,'XYZ_TRACKINGWINDOW_INFO')
        else
            data=simReadCustomDataBlock(part,'XYZ_FEEDERPART_INFO')
        end
        data=simUnpackTable(data)
    ...
    if partData['decorationInfo'] then
        ...
        local modelP=simGetObjectPosition(model,-1)
        for i=1,#allItems,1 do
            local h2=simCreateDummy(0.005)
            simSetObjectParent(h2,h,true)

            simSetObjectPosition(h2,h,allItems[i]['pos'])
            simSetObjectOrientation(h2,h,allItems[i]['orient'])
--            simSetObjectMatrix(h2,model,simBuildMatrix(allItems[i]['pos'],{0,0,0}))
--            local p=simGetObjectPosition(h2,-1)
--            simSetObjectPosition(h2,-1,{p[1]-modelP[1],p[2]-modelP[2],p[3]-modelP[3]})
            
            
            simSetObjectInt32Parameter(h2,sim_objintparam_visibility_layer,1024)
...
if (sim_call_type==sim_childscriptcall_initialization) then
    ...
    REF_INPUT=2 -- index of referenced input object handle (e.g. detection window)
    model=simGetObjectAssociatedWithScript(sim_handle_self)
    trackingWindowShape=simGetObjectHandle('genericTrackingWindow_track')
    stopLineShape=simGetObjectHandle('genericTrackingWindow_stopLine')
    local data=simReadCustomDataBlock(model,'XYZ_TRACKINGWINDOW_INFO')
    data=simUnpackTable(data)
    ...
    inputHandle=bwUtils.getReferencedObjectHandle(model,REF_INPUT)
    ...
    if conveyorHandle>=0 then
        local m=simGetObjectMatrix(conveyorHandle,-1)
    ...
    if simBoolAnd32(data['bitCoded'],2)>0 then
        console=simAuxiliaryConsoleOpen('Parts/targets in tracking window',1000,4,nil,{600,300},nil,{0.9,1,0.9})
    end
    showPoints=bwUtils.modifyAuxVisualizationItems(simBoolAnd32(data['bitCoded'],4)>0)
    overridePallet=(simBoolAnd32(data['bitCoded'],8)>0)
    stopLine=(simBoolAnd32(data['bitCoded'],16)>0)
    sphere1Container=simAddDrawingObject(sim_drawing_spherepoints,0.015,0,-1,9999,{0,1,0})
    sphere3Container=simAddDrawingObject(sim_drawing_spherepoints,0.01,0,-1,9999,{1,1,1})
    decorationContainers={}
    decorationContainers[1]=simAddDrawingObject(sim_drawing_spherepoints,0.015,0,-1,9999,{1,0,1})
    decorationContainers[2]=simAddDrawingObject(sim_drawing_spherepoints,0.015,0,-1,9999,{0,0,1})
    decorationContainers[3]=simAddDrawingObject(sim_drawing_spherepoints,0.015,0,-1,9999,{1,1,0})
    decorationContainers[4]=simAddDrawingObject(sim_drawing_spherepoints,0.015,0,-1,9999,{1,0,0})
    decorationContainerGrey=simAddDrawingObject(sim_drawing_spherepoints,0.005,0,-1,9999,{0.1,0.1,0.1})

    lineContainerR=simAddDrawingObject(sim_drawing_lines,2,0,-1,9999,{1,0,0})
    lineContainerG=simAddDrawingObject(sim_drawing_lines,2,0,-1,9999,{0,1,0})
    lineContainerB=simAddDrawingObject(sim_drawing_lines,2,0,-1,9999,{0,0,1})
...
if (sim_call_type==sim_childscriptcall_sensing) then
    ...
    simSetObjectPosition(trackingWindowShape,model,{0,-bias,height/2})
...
--]]
    ...
    for key,value in pairs(trackedParts) do
        if value['decorationInfo'] then
            ...
            for i=1,#allItems,1 do
                ...
                local p=simGetObjectPosition(h,model)
                -- ... in the window...
                if (math.abs(p[1])<width*0.5) and (p[2]>-length*0.5-bias) and (p[2]<length*0.5-bias) and (p[3]>0) and (p[3]<height) then
    ...
    simWriteCustomDataBlock(model,'XYZ_TRACKINGWINDOW_INFO',simPackTable(data))
    if stopConveyor then
        if conveyorHandle>=0 and (not stopRequest) then
            ...
            if data then
                ...
                simWriteCustomDataBlock(conveyorHandle,'CONVEYOR_CONF',simPackTable(data))
                stopRequest=true
            end
        end
    else
