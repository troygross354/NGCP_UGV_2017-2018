...
function writeInfo(data)
    if data then
        ...
        simWriteCustomDataBlock(model,'XYZ_TRACKINGWINDOW_INFO','')
    end
end
...
function getAvailableDetectionOrTrackingWindows()
    ...
    for i=1,#l,1 do
        if l[i]~=model then
            local data1=simReadCustomDataBlock(l[i],'XYZ_TRACKINGWINDOW_INFO')
            local data2=simReadCustomDataBlock(l[i],'XYZ_DETECTIONWINDOW_INFO')
...
function setSizes()
    ...
    local h=c['height']
    local slEnabled=simBoolAnd32(c['bitCoded'],16)>0
    if slEnabled then
        local r,lay=simGetObjectInt32Parameter(trackBox,sim_objintparam_visibility_layer)
        simSetObjectInt32Parameter(stopLineBox,sim_objintparam_visibility_layer,lay)
    else
        simSetObjectInt32Parameter(stopLineBox,sim_objintparam_visibility_layer,0)
    end
    setObjectSize(trackBox,w,l,h)
    simSetObjectPosition(trackBox,model,{0,0,h*0.5})
    setObjectSize(stopLineBox,w+0.005,0.005,h+0.0025)
    simSetObjectPosition(stopLineBox,model,{0,sl,h*0.5+0.0025})
    setObjectSize(transferBox,w,tl,h)
    simSetObjectPosition(transferBox,model,{0,ts+(l+tl)*0.5,h*0.5})
end

function updateEnabledDisabledItemsDlg()
    if ui then
        local config=readInfo()
        local overridePallet=simBoolAnd32(config['bitCoded'],8)~=0
        local stopLine=simBoolAnd32(config['bitCoded'],16)~=0
        ...
        simExtCustomUI_setEnabled(ui,50,enabled,true)
        simExtCustomUI_setEnabled(ui,51,enabled and stopLine,true)
        simExtCustomUI_setEnabled(ui,52,enabled and stopLine,true)
        simExtCustomUI_setEnabled(ui,18,enabled and overridePallet,true)
...
function setDlgItemContent()
    if ui then
        ...
        simExtCustomUI_setCheckboxValue(ui,50,bwUtils.getCheckboxValFromBool(simBoolAnd32(config['bitCoded'],16)~=0),true)
        simExtCustomUI_setEditValue(ui,51,string.format("%.0f",config['stopLinePos']/0.001),true)
        simExtCustomUI_setEditValue(ui,52,string.format("%.0f",config['stopLineProcessingStage']),true)
        simExtCustomUI_setEditValue(ui,24,string.format("%.0f",config['transferStart']/0.001),true)
        simExtCustomUI_setEditValue(ui,23,string.format("%.0f",config['transferLength']/0.001),true)
        ...
        simExtCustomUI_setCheckboxValue(ui,19,bwUtils.getCheckboxValFromBool(simBoolAnd32(config['bitCoded'],8)~=0),true)
...
function inputChange_callback(ui,id,newIndex)
    ...
    bwUtils.setReferencedObjectHandle(model,REF_INPUT,newLoc)
    modified=true
end
...
function createPalletDlg()
    ...
    <tabs id="77">
        ...
        palletUi=bwUtils.createCustomUi(xml,"Pallet Creation",'center',true,'onPalletCloseClicked',true--[[,closeable,onCloseFunction,modal,resizable,activate,additionalUiAttribute--]])
...
function createDlg()
    if (not ui) and bwUtils.canOpenPropertyDialog() then
        local xml =[[
                ...
                <checkbox text="Stop line (mm)" onchange="stopLine_callback" id="50" />
                <edit oneditingfinished="stopLineChange_callback" id="51"/>

                <label text="Stop line processing stage <" style="* {margin-left: 20px;}"/>
                <edit oneditingfinished="stopLineProcessingStageChange_callback" id="52"/>

                <label text="Transfer start (mm)"/>
                <edit oneditingfinished="transferStartChange_callback" id="24"/>

                <label text="Transfer length (mm)"/>
                <edit oneditingfinished="transferLengthChange_callback" id="23"/>
                ...
                <label text="Input"/>
                <combobox id="11" onchange="inputChange_callback">
                </combobox>

                <checkbox text="Override pallet creation" onchange="palletOverride_callback" id="19" />
        ...
        comboInput=customUi_populateCombobox(ui,11,loc,{},bwUtils.getObjectNameOrNone(bwUtils.getReferencedObjectHandle(model,REF_INPUT)),true,{{'<NONE>',-1}})
...
if (sim_call_type==sim_customizationscriptcall_initialization) then
    ...
    if _info['input'] then
        bwUtils.setReferencedObjectHandle(model,REF_INPUT,getObjectHandle_noErrorNoSuffixAdjustment(_info['input']))
        _info['input']=nil
    end
    ----------------------------------------
    writeInfo(_info)
    trackBox=simGetObjectHandle('genericTrackingWindow_track')
    stopLineBox=simGetObjectHandle('genericTrackingWindow_stopLine')
    transferBox=simGetObjectHandle('genericTrackingWindow_transfer')
