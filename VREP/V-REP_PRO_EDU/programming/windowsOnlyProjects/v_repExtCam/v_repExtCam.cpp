...
{ // This is called just once, at the start of V-REP
    ...
    simRegisterCustomLuaFunction(LUA_START,"number result,number resX,number resY=simExtCamStart(number deviceIndex,number resX,number resY)",inArgs1,LUA_START_CALLBACK);

    int inArgs2[]={1,sim_lua_arg_int};
    simRegisterCustomLuaFunction(LUA_END,"number result=simExtCamEnd(number deviceIndex)",inArgs2,LUA_END_CALLBACK);

    simRegisterCustomLuaFunction(LUA_INFO,"string info=simExtCamInfo(number index)",inArgs2,LUA_INFO_CALLBACK);

    int inArgs5[]={2,sim_lua_arg_int,sim_lua_arg_int};
    simRegisterCustomLuaFunction(LUA_GRAB,"number result=simExtCamGrab(number index,number visionSensorHandle)",inArgs5,LUA_GRAB_CALLBACK);
